...
public class Students {
	public static void main(String[] args) {
		...
		List<String> firstThreeChar = students.stream()
				.map(names -> names.substring(0,3))
				.collect(Collectors.toList());
		System.out.println("First three characters of name: "+firstThreeChar);
		
		// students with less than or equal to 4 char
		List<String> lessThanFiveChar = students.stream()
				.filter(names -> names.length()<=4)
				.collect(Collectors.toList());
		System.out.println("Names less than 5 character: "+lessThanFiveChar);
